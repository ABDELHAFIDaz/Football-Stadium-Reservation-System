...
<style>
  ...
  .field-grid { background-image:linear-gradient(rgba(61,255,122,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(61,255,122,.035) 1px,transparent 1px);background-size:60px 60px; }
  @keyframes slideUp { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform:translateY(0)} }
  .anim { animation:slideUp .55s ease both; }
  .glow { box-shadow:0 0 32px rgba(61,255,122,.2); }
  .glow:hover { box-shadow:0 6px 40px rgba(61,255,122,.3); }
  input[type=radio]:checked + label { border-color:#3DFF7A!important; background:rgba(61,255,122,.06)!important; }
</style>
</head>
  ...
  <div class="w-full max-w-[420px] anim">
    ...
    <form  action="{{ route('register') }}" class="space-y-4">
      ...
      <div>
        ...
        <input type="password" placeholder="Repeat password" autocomplete="new-password" name="confirm-password"
          class="w-full bg-surface border border-neon/10 rounded-xl px-4 py-3 text-white text-[.87rem] font-dm outline-none placeholder-white/20 focus:border-neon/40 focus:ring-2 focus:ring-neon/5 transition-all">
      </div>

      <!-- TERMS -->
      <div class="flex items-start gap-3 pt-1">
        <input type="checkbox" id="terms" class="w-4 h-4 mt-0.5 accent-neon flex-shrink-0 cursor-pointer rounded">
        <label for="terms" class="text-[.79rem] text-white/35 leading-relaxed cursor-pointer">
          I agree to the <a href="#" class="text-neon hover:underline">Terms of Service</a> and <a href="#" class="text-neon hover:underline">Privacy Policy</a> of BALLe.
        </label>
      </div>
...
<div class="relative bg-surface border-l border-neon/10 flex flex-col justify-center px-16 py-20 overflow-hidden">
  ...
  <div class="relative z-10 bg-card border border-neon/10 rounded-2xl p-6 max-w-[340px]">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="font-syne font-bold text-[.81rem] text-white">Karim Mansouri</div>
...
function checkStrength(val) {
  const b1=document.getElementById('b1'),b2=document.getElementById('b2'),b3=document.getElementById('b3'),lbl=document.getElementById('pwlbl');
  ...
  if (score===1) { b1.style.background='#FF5757'; lbl.textContent='Weak'; lbl.style.color='#FF5757'; }
  else if (score===2) { b1.style.background='#FFB800'; b2.style.background='#FFB800'; lbl.textContent='Medium'; lbl.style.color='#FFB800'; }
  else { b1.style.background='#3DFF7A'; b2.style.background='#3DFF7A'; b3.style.background='#3DFF7A'; lbl.textContent='Strong'; lbl.style.color='#3DFF7A'; }
}
</script>
</body>
</html>